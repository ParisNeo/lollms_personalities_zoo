...
class Processor(APScript):
    ...
    def __init__(
        ...
        personality_config_template = ConfigTemplate(
            [
                ...
                {"name":"header_template", "type":"text", "value":"# author : put the user name here\n# date: put the date here\n# description:\nPut the description here", "help":"Header to add to all your commented files."},
    ...
    def run_workflow(self,  context_details:dict=None, client:Client=None,  callback: Callable[[str | list | None, MSG_OPERATION_TYPE, str, AIPersonality| None], bool]=None):
        ...
        infos = f"Date time : {datetime.now()}"
        if self.personality_config.header_template!="":
            infos += "\n".join([
                self.system_custom_header("file header template"),
