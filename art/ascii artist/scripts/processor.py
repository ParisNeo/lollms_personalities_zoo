...
from lollms.utilisites import PackageManager
import re
import importlib
import requests
...
class AsciiArtConverter:
    ...
    def _pixels_to_ascii(self, image):
        ...
        for pixel_value in pixels:
            ascii_str += self.ASCII_CHARS[pixel_value // 25]
        return ascii_str
    ...
    def generate_ascii_art(self):
        ...
        image = self._grayify(image)
        ascii_str = self._pixels_to_ascii(image)

        # Separate the ASCII art into lines with the desired width
        pixel_count = len(ascii_str)
        ascii_art = "\n".join([ascii_str[index : index + self.new_width] for index in range(0, pixel_count, self.new_width)])
        return ascii_art
...
class Processor(APScript):
    ...
    def show_last_image(self, prompt, full_context):
        ...
        if len(self.image_files)>0:
            self.full(f"![]({self.image_files})", callback=self.callback)        
        else:
            self.full("Showing Stable diffusion settings UI", callback=self.callback)        
