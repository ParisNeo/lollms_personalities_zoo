        ...
        if index==0:# "The prompt is asking for creating a new index"
            ...
            code = "\n".join([
                ...
                "\n".join([
                    ...
                    f"{self.config.start_header_id_template}instructions: Make a python function that takes an ElasticSearch object es and perform the right operations to add an entry to the database as asked by the user. The output should be in form of a boolean.",
                    f"{self.config.start_header_id_template}mapping:{mapping}\nHere is the signature of the function:\ndef add_entry(es:ElasticSearch, index_name):",
                    f"Do not provide explanations or usage example.",
                    f"{self.config.start_header_id_template}elasticsearch_ai:Here is the query function that you are asking for:",
                    "```python",
                    "from elasticsearch import Elasticsearch\n"
                ]), callback=self.sink)
