...
class Processor(APScript):
    ...
    def run_workflow(self, prompt:str, previous_discussion_text:str="", callback: Callable[[str | list | None, MSG_OPERATION_TYPE, str, AIPersonality| None], bool]=None, context_details:dict=None, client:Client=None):
        ...
        templates = [
            "An endpoint",
            "Text to speach service template",
            "None of these",
        ]
        self.step_start("Choosing template")
        choice = self.multichoice_question("which template do you require to help ParisNeo?",templates)
        self.step_end("Choosing template")
        self.step(f"Chosen {templates[choice]}")
        if choice==0:
            # Define the path to the file
            template_code = self.read_template('example_python_endpoint.py')
        elif choice==1:
            # Define the path to the file
            template_code = self.read_template('example_tts_service.py')
        # Assuming context_details and callback are already defined
        full_prompt = self.build_prompt_from_context_details(context_details, "# template to be used to build the code:\n" + template_code)
